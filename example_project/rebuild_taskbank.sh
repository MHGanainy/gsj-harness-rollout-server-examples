...
# estate/corpus/staging/corpus.lock.json `taskbank` block). The generator is the
...
"$LIB/.venv/bin/python" "$LIB/estate/corpus/ingest_corpus.py" taskbank --corpus "$LIB/estate/corpus/staging"
cp "$LIB/estate/corpus/staging/taskbank.parquet" taskbank.parquet

echo "lock sha256: $("$LIB/.venv/bin/python" -c \
  "import json; print(json.load(open('$LIB/estate/corpus/staging/corpus.lock.json'))['taskbank']['sha256'])")"
